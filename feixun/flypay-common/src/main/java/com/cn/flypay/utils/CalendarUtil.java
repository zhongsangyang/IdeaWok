 ...
 * @author sunny.chen@logyi.com
 * @date 2016年7月7日 下午3:42:35
 * 
 */
public class CalendarUtil {
	...
	public static List<String> getPre5Years() throws Exception {
	...
	public static String getNextDay(String format) {
		...
		return DateUtil.getDateTime(format, calendar.getTime());
	}
	...
	public static Map<String, String> getMonthFirstEndDayTime(String year, String month) {
		...
		String firstDayTime = DateUtil.getDateTime("yyyy-MM-dd", calendar.getTime()) + " 00:00:00";

		calendar.set(Calendar.DATE, calendar.getActualMaximum(Calendar.DATE));// 设置本月最大日期
		String lastDayTime = DateUtil.getDateTime("yyyy-MM-dd", calendar.getTime()) + " 23:59:59";
	...
	public static List<String> getBetweenMonths(String startMonth, String endMonth) {
		...
		while (startCalendar.getTimeInMillis() <= endCalendar.getTimeInMillis()) {
			months.add(DateUtil.getDateTime("yyyy-MM", startCalendar.getTime()));
	...
	public static List<String> getBetweenDateStr(String startTime, String endTime) {
		...
		try {
			Long startM = DateUtil.getDateFromString(startTime).getTime();
			Long endM = DateUtil.getDateFromString(endTime).getTime();
			...
			for (int i = 0; i <= result; i++) {
				startCalendar.add(Calendar.DATE, 1);
				dateList.add(DateUtil.getDateTime("yyyy-MM-dd", startCalendar.getTime()));
			}
	...
	public static List<String> getBetweenDateStr(String startTime, String endTime, String format) {
		...
		try {
			...
			for (int i = 0; i <= result; i++) {
				startCalendar.add(Calendar.DATE, 1);
				dateList.add(DateUtil.getDateTime(format, startCalendar.getTime()));
			}
		} catch (ParseException e) {
			e.printStackTrace();
		} catch (Exception e) {
	...
	public static List<String> getPreNMonths(String curMonth, Integer n) {
		...
		while (tmpCalendar.getTimeInMillis() <= curCalendar.getTimeInMillis()) {
			months.add(DateUtil.getDateTime("yyyy-MM", tmpCalendar.getTime()));
	...
	public static boolean isDayEqual(Date date1, Date date2) {
		...
		String date1Str = DateUtil.getDateTime("yyyy-MM-dd", date1);
		String date2Str = DateUtil.getDateTime("yyyy-MM-dd", date2);
		return date1Str.equals(date2Str);
	}

	/**
	 * 时间间距是否为xx； 如果在 space 之内返回true；否则返回false
	 * 
	 * @param startTime
	 *            开始时间
	 * @param endTime
	 ...
	 * @return
	 */
	public static boolean isTimeSpace(Date startTime, Date endTime, long space) throws Exception {
