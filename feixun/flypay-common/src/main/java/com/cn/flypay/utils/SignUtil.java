...
public class SignUtil {
	...
	/*public static String getSign(Map<String, String> map) {
		...
		result = result + "key=" + (String) ApplicatonStaticUtil.getAppStaticData("wxaccount.appPaySecret");

		result = Digests.md5Hex(result).toUpperCase();
		return result;
	}*/

	public static String getSign(Map<String, String> map, String wxAppPaySecret) {
		List<String> list = new ArrayList<String>();
		for (Map.Entry<String, String> entry : map.entrySet()) {
			if (StringUtil.isNotEmpty(entry.getValue())) {
				list.add((String) entry.getKey() + "=" + entry.getValue() + "&");
		...
		result = result + "key=" + wxAppPaySecret;
