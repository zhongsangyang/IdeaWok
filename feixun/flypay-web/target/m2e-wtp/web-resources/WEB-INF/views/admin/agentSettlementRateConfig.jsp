...
<script type="text/javascript">
	...
	$(function() {
		...
		dataGrid = $('#dataGrid')
				.datagrid(
						{
							url : '${ctx}/agentSettlementRate/dataGrid',
							fit : true,
							striped : true,
							rownumbers : true,
							pagination : true,
	...
	function editFun(id, agentType, payType) {
		if (id == undefined) {
			var rows = dataGrid.datagrid('getSelections');
			id = rows[0].organization.id;
			agentType = rows[0].agentType;
			payType = rows[0].payType;
		} else {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 300,
			href : '${ctx}/agentSettlementRate/editPage?orgId=' + id
					+ '&agentType=' + agentType + '&payType=' + payType,
			buttons : [ {
				text : '编辑',
				handler : function() {
