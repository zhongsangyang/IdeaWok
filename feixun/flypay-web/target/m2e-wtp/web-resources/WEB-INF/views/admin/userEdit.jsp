...
<script type="text/javascript">
	$(function() {
		...
		$('#roleIds').combotree({
			...
			required: true,
			cascadeCheck : false,
			queryParams　: {'roleIdStr':'${user.roleIds}'},
		...
		<form id="userEditForm" method="post">
			...
			<table class="grid">
				...
				<tr>
					<td>用户类型</td>
					<td>
					<c:if test="${user.userType == 24}">普通用户</c:if>
					<c:if test="${user.userType == 23}">银牌用户</c:if>
					<c:if test="${user.userType == 22}">金牌用户</c:if>
					<c:if test="${user.userType == 21}">钻石用户</c:if>
					</td>
				</tr>
				<tr>
					<td>登录名</td>
				...
				<tr>
					<td><font style="color: red;">是否迁移下级(慎重)</font></td>
