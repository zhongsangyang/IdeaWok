...
public class NewUserMobileControllerTest {
	private static Log log = LogFactory.getLog(NewUserMobileControllerTest.class);

	private static String token = "8kh0ufovjok8";
	...
	public void testApplyAuthentication() {
		...
		try {
			...
			String cardType = "J";
			String chk = MD5Util.md5(merId.trim() + idNo.trim() + reservedPhone.trim() + cardNo.trim()
					+ openBankId.trim() + "flypaysmrz");
			JSONObject json = new JSONObject();
			json.put("merId", merId);
			json.put("idNo", idNo);
	...
	public void testUpdateSettlementBankCard() {
		...
		try {
			...
			String cardType = "J";
			String chk = MD5Util
					.md5(merId.trim() + reservedPhone.trim() + cardNo.trim() + openBankId.trim() + "flypayjskgx");
			JSONObject json = new JSONObject();
			json.put("merId", merId);
			json.put("idNo", idNo);
			json.put("cardNo", cardNo);
	...
	public void testAddBankCard() {
		...
		try {
			...
			String cardType = "J";
			String chk = MD5Util.md5(merId.trim() + cardNo.trim() + openBankId.trim() + reservedPhone.trim()
					+ cardType.trim() + "flypaytjyhk");
			System.out.println(merId.trim() + idNo.trim() + reservedPhone.trim() + cardNo.trim() + openBankId.trim()
					+ "flypayjskgx");
			JSONObject json = new JSONObject();
			json.put("merId", merId);
			json.put("idNo", idNo);
	...
	public void testqueryOrderList() {
		try {
			...
			json.put("pageNum", "0");
			json.put("startDate", DateUtil.convertDateStrYYYYMMDD(DateUtil.getBeforeDate(new Date(), 30)));
			json.put("endDate", DateUtil.convertDateStrYYYYMMDD(new Date()));
			json.put("pageSize", "5");
			json.put("appType", "ios");
	...
	private static String httpJsonRequest(String requestUrl, String requestMethod, String outputStr) throws Exception {
		...
		try {
			...
			if (bs.startsWith("{\"flypayContent\"")) {
				log.info(
						URLDecoder.decode(
								MsgInfoDESUtil.DecryptDoNet(
										bs.substring("{\"flypayContent\":\"".length(), bs.lastIndexOf("\"}"))),
						"utf-8"));
			} else {
	...
	public void testconsumeUserPointByReduceChlRate() {
		...
		try {
			...
			json.put("agentId", "F20160001");
			json.put("appType", "ios");
			json.put("chlType", "200");
			json.put("consumePoint", "2");
			json.put("type", "3");
			json.put("transPwd", MD5Util.md5("123456"));
			json.put("chkValue",
					MD5Util.md5(json.getString("merId") + json.getString("agentId") + json.getString("chlType")
							+ json.getString("consumePoint") + json.getString("type") + json.getString("transPwd")
							+ "flypayjfxf"));
